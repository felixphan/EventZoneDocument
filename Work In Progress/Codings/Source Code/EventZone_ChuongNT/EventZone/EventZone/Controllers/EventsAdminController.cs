...
namespace EventZone.Controllers
{
    public class EventsAdminController : Controller
    {
        ...
        public ActionResult SearchEvent(string eventName, string userName, string categoryName)
        {
            var events = db.Events.Include(e => e.Category).Include(e => e.Channel);
          
            if (String.IsNullOrEmpty(eventName)==false&&String.IsNullOrEmpty(userName)==false)
            {

                events = events.Where(e => e.EventName.Contains(eventName) && e.Channel.User.UserName.Contains(userName) && e.Category.CategoryName.Equals(categoryName));
                     //)&& e.Category.CategoryName.Equals(categoryName));
             
            }else
            {
                if (String.IsNullOrEmpty(eventName) == true && String.IsNullOrEmpty(userName) == false)
                {
                    if (categoryName.Equals("All category"))
                        events = events.Where(e => e.Channel.User.UserName.Contains(userName));
                    else
                        events = events.Where(e => e.Channel.User.UserName.Contains(userName) && e.Category.CategoryName.Equals(categoryName));
                }
                else
                {
                    if (String.IsNullOrEmpty(eventName) == false && String.IsNullOrEmpty(userName) == true)
                        if (categoryName.Equals("All category"))
                            events = events.Where(e => e.EventName.Contains(eventName));
                        else
                        events = events.Where(e => e.EventName.Contains(eventName)&&e.Category.CategoryName.Equals(categoryName));
                    else
                        if (!categoryName.Equals("All category"))
